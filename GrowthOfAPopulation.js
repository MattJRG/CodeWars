...
function nbYear(initialPop, percent, variable, targetPop) {
  // create a variable for the year
  let year = 0;
  // create a variable for currPop
  let currPop = initialPop;
  // create decimal variable;
  let decimal = percent/100;
  // loop while the currPop is < targetPop incrementing the year
  while (currPop < targetPop) {
    currPop = Math.floor(currPop + currPop * decimal + variable);
    year++
  }
  return year;
}
